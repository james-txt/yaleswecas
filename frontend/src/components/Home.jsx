...
function Home() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      const data = await getDataFromServer(
        `${import.meta.env.VITE_APIURL}/get_ids`
      );
  ...
  return (
    <div className="card flex flex-col items-center justify-center my-2">
      ...
      <ul data-testid="idlist" className="grid grid-cols-2 gap-2 w-11/12">
        {ids.map((id, index) => (
          <li key={index}>
            <Link
              to={`/details/${id}`}
              className="inline-block py-2 px-12 rounded bg-slate-200 hover:bg-slate-50 active:bg-slate-400 font-medium"
            >
              {id}
            </Link>
          </li>
