...
function Details() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      const data = await getDataFromServer(
        `${import.meta.env.VITE_APIURL}/get_details`,
        {
          method: "POST",
  ...
  return details ? (
    <dl className="card w-11/12 lg:w-4/5 xlg:w-3/4 flex flex-col items-center justify-center">
      ...
      <Link
        to="/"
        className="text-slate-800 hover:text-slate-600 font-medium py-1 mt-3 px-4 rounded bg-slate-200 hover:bg-slate-50 active:bg-slate-400"
      >
        Back
      </Link>
    </dl>
